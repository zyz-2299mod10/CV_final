...
def parse_args():
    ...
    parser.add_argument('--add_GN', action='store_true', default=False, help='add gaussian noise to depth img')
...
def main(args):
    ...
    # elif args.data == 'my':
    #     data_root = os.path.join('/home/hcis/YongZhe/CFVS_data_new', 'processed')

    data_root = os.path.join('./far_view_nonoise_crop', 'processed')
    ...
    for key, value in tqdm.tqdm(list(data.items())):
        ...
        for idx, depth_image_filename in enumerate(depth_image_filename_list):
            ...
            if args.add_GN:
                depth_img, noise = add_gaussian_noise(depth_img, sigma=0.002, mean=0)
        ...
        if args.aug_data:
            ...
            concat_xyz_in_world = jitter_point_cloud(concat_xyz_in_world, sigma=0.5, clip=3)
        ...
        if args.crop_pcd == True:
            ...
            maintain_points = 2400
            if concat_xyz_in_world.shape[0] > maintain_points:
                concat_xyz_in_world, drop_idx = specific_point_dropout(concat_xyz_in_world, drop_num=concat_xyz_in_world.shape[0]-maintain_points)
                seg_label = np.delete(seg_label, drop_idx, axis=0)
                seg_color = np.delete(seg_color, drop_idx, axis=0)
            elif concat_xyz_in_world.shape[0] < maintain_points:
                data.pop(key, None)
                print('pop key: ', key)
                continue
        ...
        if args.offset == 'kpts':
            ...
            hole_keypoint_top_x_pos_in_world = hole_keypoint_top_pos_in_world - hole_x_vec * 0.005
            hole_keypoint_top_y_pos_in_world = hole_keypoint_top_pos_in_world - hole_y_vec * 0.0025
